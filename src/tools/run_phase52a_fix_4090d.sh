...
root="work_dirs/phase52a_fix_shadow_status_1pct"
config="configs/protocol/phase52a_fix_shadow_status_1pct_l1c.py"
work_dir="${root}/seed52"
baseline_checkpoint="work_dirs/phase22_clean_v8/seed42/best_mIoU_iter_40000.pth"
mkdir -p "${root}"

python tools/audit_phase52a_fix.py
...
if [[ "${#checkpoints[@]}" -ne 1 ]]; then
  echo "Expected one best Phase 52A-fix checkpoint, found ${#checkpoints[@]}" >&2
  exit 3
fi
...
python tools/evaluate_phase52a_fix.py --checkpoint "${checkpoints[0]}"
