...
root="work_dirs/phase37_weakclass_supervision"
...
phase36_summary="work_dirs/phase36_mobilenetv3_litefpn/summary.json"
resume_args=()

python - "${phase31_summary}" "${phase36_summary}" <<'PY'
import json
from pathlib import Path
import sys
p31 = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
p36 = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
if p31.get("phase") != 31 or p31.get("passed") is not True:
    raise SystemExit("Phase 37 requires Phase 31 benchmark evidence")
if p36.get("phase") != 36 or p36.get("passed") is not False:
    raise SystemExit("Phase 37 requires the failed Phase 36 stop-loss")
if p36.get("internal_test_evaluated") is not False:
...
python tools/summarize_phase37_weakclass_supervision.py \
