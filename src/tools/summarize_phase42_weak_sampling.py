...
def main():
    ...
    if phase31.get("phase") != 31 or phase31.get("passed") is not True:
        raise SystemExit("Phase 42 requires the frozen V8 benchmark from Phase 31")
    if manifest.get("phase") != 42 or manifest.get("sample_count") != 8490:
        raise SystemExit("Invalid Phase 42 sampling manifest")
    evaluation = parse_last_evaluation(root / "seed42" / "val_eval.log")
    metrics, per_class = evaluation["aggregate"], evaluation["per_class"]
    ...
    finite = all(math.isfinite(float(value)) for value in
                 [*metrics.values(), weak_miou, speedup, parameters_m])
    gates = {
    ...
    result = {
        ...
        "seed": 42,
        "sampling": {key: manifest[key] for key in
                     ("sample_count", "rich_fraction", "rich_count",
                      "rich_probability", "weak_classes",
                      "annotation_sha256", "minimum_rich_prevalence")},
