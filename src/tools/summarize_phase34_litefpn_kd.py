...
def main():
    ...
    baseline_miou = float(phase31["validation"]["mIoU"])
    improvement = float(metrics["mIoU"]) - baseline_miou
    benchmark = json.loads((root / "seed42" / "benchmark.json").read_text())
    baseline_benchmark = phase31["baseline_benchmark"]
    speedup = float(baseline_benchmark["latency_mean_ms"]) / float(
        benchmark["latency_mean_ms"]
    )
    parameters_m = float(benchmark["parameters_m"])
    finite = all(
        math.isfinite(float(value))
        for value in [*metrics.values(), weak_miou, improvement, speedup, parameters_m]
    )
    gates = {
        "validation_mIoU": float(metrics["mIoU"]) >= args.min_miou,
        "paired_improvement": improvement >= args.min_improvement,
    ...
    result = {
        ...
        "validation": metrics,
        "per_class": per_class,
        "phase31_seed42_mIoU": baseline_miou,
