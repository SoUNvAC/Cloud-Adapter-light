...
root="work_dirs/phase47_pixel_factorized_v8"
work_dir="${root}/seed42"
config="configs/protocol/phase47_pixel_factorized_v8_l1c.py"
report="${root}/PHASE47_REPORT.txt"
mkdir -p "${root}"
: > "${report}"
exec > >(tee -a "${report}") 2>&1
finish_report() {
  status=$?; trap - EXIT; set +e
  echo; echo "===== PHASE 47 FINAL STATUS ====="; echo "PIPELINE_EXIT_CODE=${status}"
...
row = json.loads(Path("work_dirs/phase46_factorized_v8/summary.json").read_text())
if row.get("passed") is not False or row.get("decision") != "move_factor_heads_to_pixel_features":
    raise SystemExit("Phase 46 stop-loss decision is required")
if row["gates"].get("target_test_evaluated") is not False:
    raise SystemExit("Target test is not sealed")
PY

echo "===== PHASE 47 PIXEL-FACTOR RUN ====="
...
if [[ -e "${work_dir}" ]]; then
  ...
  if [[ "${#completed[@]}" -eq 1 && -f "${work_dir}/TRAIN_COMPLETE" ]]; then
    echo "Reusing completed Phase 47 training"
  elif [[ -f "${work_dir}/last_checkpoint" ]]; then
    resume_args=(--resume)
  elif ! find "${work_dir}" -maxdepth 2 -type f -name '*.pth' -print -quit | grep -q .; then
    echo "Restarting zero-checkpoint Phase 47 directory"
  else
    echo "Refusing checkpoint-bearing Phase 47 run without last_checkpoint" >&2; exit 2
  fi
fi
if [[ ! -f "${work_dir}/TRAIN_COMPLETE" ]]; then
  ...
  [[ "${#checkpoints[@]}" -eq 1 ]] || exit 3
  touch "${work_dir}/TRAIN_COMPLETE"
fi
mapfile -t checkpoints < <(find "${work_dir}" -maxdepth 1 -type f -name 'best_mIoU_iter_*.pth' -print)
[[ "${#checkpoints[@]}" -eq 1 ]] || exit 4
python tools/evaluate_phase46_factorized.py \
  --phase 47 \
  --method frozen_v8_pixel_feature_factor_residual \
  --trainable-parameters 387 \
  --config "${config}" \
  --output-root "${root}/eval" \
