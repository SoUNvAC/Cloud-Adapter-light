...
root="work_dirs/phase42_weak_sampling"
work_dir="${root}/seed42"
config="configs/protocol/phase42_mobilenetv2_weak_sampling_l1c.py"
...
phase41_summary="work_dirs/phase41_detail_litefpn/summary.json"
resume_args=()

python - "${phase41_summary}" <<'PY'
import json
from pathlib import Path
import sys
p41 = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if p41.get("phase") != 41 or p41.get("passed") is not False:
    raise SystemExit("Phase 42 requires the failed Phase 41 stop-loss")
if p41.get("internal_test_evaluated") is not False:
...
if [[ ! -f "${root}/sampling_manifest.json" ]]; then
  python tools/prepare_phase42_sampling_manifest.py \
    --ann-dir "${CLOUD_ADAPTER_DATA_ROOT}/ann_dir/train" \
    --expected-samples 8490 --rich-fraction 0.25 \
    --output "${root}/sampling_manifest.json"
fi
...
python tools/summarize_phase42_weak_sampling.py \
