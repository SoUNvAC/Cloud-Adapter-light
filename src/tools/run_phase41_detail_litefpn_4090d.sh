...
root="work_dirs/phase41_detail_litefpn"
work_dir="${root}/seed42"
config="configs/protocol/phase41_mobilenetv2_detail_litefpn_l1c.py"
...
phase40_summary="work_dirs/phase40_error_audit/summary.json"
resume_args=()

python - "${phase40_summary}" <<'PY'
import json
from pathlib import Path
import sys
p40 = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if p40.get("phase") != 40 or p40.get("passed") is not True:
    raise SystemExit("Phase 41 requires the completed Phase 40 audit")
if p40.get("selected_direction") != "high_resolution_boundary_preserving_architecture":
    raise SystemExit("Phase 40 did not select the Phase 41 method")
if p40.get("internal_test_evaluated") is not False:
...
python tools/summarize_phase41_detail_litefpn.py \
